...
module.exports = {
  async updateLedger(dateFrom, party) {
    await bookshelf.transaction(async (t) => {
      ...
      await utils.asyncForEach(monthDiffArray, async (m_no) => {
        ...
        let getMonthlySaleBalance = await strapi
        ...
        if (getMonthlySaleBalance && getMonthlySaleBalance.length) {
          getMonthlySaleBalance.forEach((el) => {
            let balance = {
              opening_balance: validateNumber(el.opening_balance),
              closing_balance: validateNumber(el.opening_balance),
            };
            if (el.sale_type === "Pakka") {
              pakkaLedgerId = el.id;
        ...
        getAllTransactions.forEach((el) => {
          ...
          //   if (el.is_payment || el.is_sale_return) {
          //     num = -num;
          //   }
        ...
        /** For Kachha ledger */
        if (kachhaLedgerId) {
          await strapi.query("monthly-sale-balance").update(
            { id: kachhaLedgerId },
            {
