...
import form from "./form/Color.json";
...
export default function AddEditColor(props) {
  ...
  const handSubmit = async () => {
    ...
    if (isEdit) {
      await providerForPut(backend_color, formState.id, data, Auth.getToken())
        .then(res => {
          history.push(COLORS);
          setLoading(false);
        })
      ...
      await providerForPost(backend_color, data, Auth.getToken())
        .then(res => {
          history.push(COLORS);
          setLoading(false);
        })
